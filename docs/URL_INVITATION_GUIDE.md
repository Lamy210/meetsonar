...
MeetSonarに新しくURL招待機能を追加しました。この機能により、URLパラメータを使用して招待情報を事前設定し、よりスムーズな参加体験を提供できます。

## 機能概要

### 1. URL招待とは
- URLパラメータに招待情報を含めることで、受信者の参加プロセスを簡素化
- 招待者情報、表示名、メールアドレスなどを事前設定
- トークンベースの個人招待と併用可能

### 2. 主な特徴
- **簡単共有**: URLをコピーして任意の方法で共有
- **事前設定**: 招待される人の情報を事前に設定
- **自動ウェルカム**: 参加時に招待者情報を表示
- **有効期限**: パラメータで有効期限を設定可能

## URL構造

### 基本形式
```
https://meetsonar.example.com/join/{roomId}?[parameters]
```
...
| パラメータ | 説明 | 例 |
|----------|------|-----|
| `inviter` | 招待者の表示名 | `inviter=田中太郎` |
| `email` | 招待される人のメールアドレス | `email=user@example.com` |
| `name` | 招待される人の推奨表示名 | `name=佐藤花子` |
| `message` | カスタムメッセージ (将来実装) | `message=会議に参加してください` |
| `expires` | 有効期限 (ISO 8601形式) | `expires=2025-07-25T10:00:00Z` |

### 実際のURL例
```
https://meetsonar.example.com/join/room123?inviter=田中太郎&email=user@example.com&name=佐藤花子&expires=2025-07-25T10:00:00Z
```
